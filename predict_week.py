...
from project_config import CURRENT_PL_TEAMS, internal_team_name
...
# ---------------------------------------------------------
#
# Some historical/current Football-Data.co.uk files use shorter names
# than football-data.org. Keep one Elo identity for each real club.
...
def normalize_team_name(team: str) -> str:
    """Return one canonical project name for a team."""
    canonical = internal_team_name(team)
    return LEGACY_TEAM_NAME_ALIASES.get(canonical, canonical)
...
# Normalize historical team names before Elo/form reconstruction.
...
# Normalize fixture team names to the same canonical names used in history.
...
# ---------------------------------------------------------
# SAVE PERMANENT PREDICTION HISTORY
# ---------------------------------------------------------

PREDICTION_HISTORY_FILE = (
    DATA_FOLDER / "prediction_history.csv"
)
...
if PREDICTION_HISTORY_FILE.exists():
    ...
    prediction_history = new_history.copy()
...
if "Matchweek" in prediction_history.columns:
    dedupe_columns.insert(
        1,
        "Matchweek",
    )

elif "Date" in prediction_history.columns:
    dedupe_columns.insert(
        1,
        "Date",
    )
...
print(
    f"Prediction history saved to: "
    f"{PREDICTION_HISTORY_FILE.resolve()}"
)
